...
Hash_l* getRehash(Btree* btree, size_t size)
{
	...
	hash = createRehash(size);

	convertBtree(btree, hash, insertRehash);
...
void insertRehash(void* h, void* w)
{
	Hash_l* hash = h;
	Word* word = w;
	size_t index, index2, saveindex;

	index = hash_function(word->word) % hash->size;
	index2 = hash_2_function(word->word) % hash->size;
	...
	if(hash->table[index] != NULL)
	{
		fprintf(stderr, "Hash REHASH FULL !\n");  /* Como o load factor é 70%, este caso não irá ocorrer. */
		exit(EXIT_FAILURE);
	}
...
Word* searchRehash(void* haystack, char* string)
{
	...
	index2 = hash_2_function(string) % hash->size;
